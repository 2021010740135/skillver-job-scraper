...
## TODO
1.兼容浏览器Edge、Chrome
2.用户输入岗位与58个岗位不匹配（先规则匹配、然后大模型语义匹配），怎么映射到58个岗位并且准确？->可以简历一个测试集
3.断点续爬，读取上次“任务状态”
4.解耦脚本
5.优化REDEME、SKILL.MD
6.我合并你的STAR.MD
7.阉割掉UCSS、不做数据分析：暂时把边界定位岗位统计、目标是更稳（爬虫稳定不容易挂、工作流稳定）、更准（用户自然语言描述映射58个岗位更准，脚本爬取到列表之后映射的更准：这里维护一个评测集）、不用太快
