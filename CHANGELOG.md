...
- **定岗彻底简化为 Agent 语义判断**：去掉规则表匹配逻辑
  - 流程：Agent 先判用户需求是否属于采集范围（判据见 SKILL.md「采集边界」）→ 范围外**明确拒绝**、不硬映射；边界模糊询问用户或宁缺毋滥 → 范围内**直接输出唯一最终答案**（`position_catalog.json` 原名之一）作为 `--position-name` 与搜索词
  - 不再有「规则匹配 → 最长别名消歧 → 歧义人工确认回写 → 零命中语义映射」的多级分支
- `position_aliases.json` 规则表**保留文件**，但不再用于定岗（标注"保留作参考/评测"）
- `SKILL.md`：Step 1 定岗重写（范围判断 → 输出唯一最终答案）、执行原则第 8 条、description、资源表、「不要遗漏」同步
- `README.md`：定岗小节与特性同步
...
- **定岗改为「先范围、再规则、后语义」**：不再强制把用户输入映射到 58 岗
  - 入口先判「是否属于采集范围」：岗位须围绕 **AI 模型 / 智能体(Agent) / 大模型 / 机器人 / 具身智能技术及其产品全链路**（算法研发、AI 应用、数据、MLOps、安全合规、AI 产品、方案售前交付、AI 商业化等 12 族）
  - **范围外明确拒绝**（通用 Java 后端、普通销售运营、行政财务、设计剪辑等，无论叫法多像「工程师」），**不硬映射**；边界模糊宁可拒绝或询问用户（宁缺毋滥）
  - 拒绝结果**不回填**规则表（范围外不是同义叫法）
- `SKILL.md`：Step 1 定岗匹配重写（范围判断入口 + 拒绝优先）+ 新增「采集边界」小节（判据/范围内外信号/要点）；执行原则第 8 条与「不要遗漏」同步
- `README.md`：定岗小节同步「用户需求 → 范围内 58 岗（或明确拒绝）」
