# start.md — skillver-job-scraper 安装契约

| 项 | 值 |
|---|---|
| 文档状态 | 生效（Active） |
| 适用版本 | 与仓库版本一致（见 `SKILL.md` frontmatter `version`） |
...
**本契约不覆盖**：登录、USCC 核验、CSV 核验——这三个人机闸门属于抓取流程，安装阶段不涉及。
...
| USCC 核验 | 属于抓取流程，安装阶段不涉及；不得代替用户确认 |
| CSV 核验 | 同上 |
